function Ys = BPAD_RR_V2_SR(Xs, SR_config)
...
for p = 1:P
    ...
    % Find the Kinit closest neighbours from the low-resolution image
    idx  = nearest_neighbours(L,xp_s,Kinit);
    
    % Derive the sub-dictionary to be considered
    L_tilda = L(:,idx); H_tilda = H(:,idx);
        
    % Find the projection matrix
    Phi_tilda = H_tilda * L_tilda' / (L_tilda * L_tilda' + lambda*eye(size(L_tilda,1))); % Use all elements in dictionary
