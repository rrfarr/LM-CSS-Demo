function Ys = CCA_BP_RR_SR(Xs, SR_config)
...
for p = 1:P
    ...
    % Determine the CCA projection matrices
    PH = SR_config.SR_dictionary.PH{p};
    PL = SR_config.SR_dictionary.PL{p};
    
    % Project the low resolution dictionary on the common sub-space
    H_c = PH' * H;
    
    % Project the input sample on the common sub-space
    x_c = PL' * xp_s;
    
    % Derive the indexes using sparse coding
    idx_SC = sparselab_BP(H_c,x_c,delta,tau,Smin);
